...
#####如何根据手势滑动来控制这段曲线？这个问题也是实现此效果的核心，当然也是很简单的，简单的说下思路。首先我们要重写已经玩烂的onTouchEvent()方法,在 ACTION_DOWN中记录下手指按下的Y坐标，在ACTION_MOVE中算出实时的距离：就是当前手指的位置 - 初始按下的位置 ，就像这样然后我们将上面介绍过的贝塞尔曲线画法封装一下就像这样：因为需求确定，起始点就定在了屏幕的左上角，终点就定在屏幕的右上角，只需传入控制点即可但是我们如果无限的向屏幕下方滑动那么贝塞尔曲线的弧度也就越大，是不是感觉弧度太大也不是很美观，所以就要对他进行约束。这个约束条件就是当我们滑动Y的距离超过屏幕的1/10时就让这个距离固定在屏幕高度的1/10，如果小于1/10那么就根据我们手指移动的距离来与用户交互。最后我们在onDraw()方法中调用它就行了，需要强调的是最后一定要调用canvas中的drawPath()方法，这个Path参数就是我们生成的贝塞尔曲线。
